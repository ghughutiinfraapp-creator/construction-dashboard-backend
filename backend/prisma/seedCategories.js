...
async function main() {
  ...
  for (const phase of PHASES) {
    ...
    const parent = existingPhase
      ? await prisma.taskCategory.update({ where: { id: existingPhase.id }, data: { order: phase.order } })
      : await prisma.taskCategory.create({ data: { name: phase.name, order: phase.order } });
    ...
    for (let i = 0; i < phase.items.length; i++) {
      ...
      if (existingItem) {
        await prisma.taskCategory.update({ where: { id: existingItem.id }, data: { order: i + 1 } });
      } else {
        await prisma.taskCategory.create({ data: { name: phase.items[i], order: i + 1, parentId: parent.id } });
      }
    }
